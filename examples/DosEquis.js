...
bot.onCommand("fallback", function (command) {
    ...
    if (command.message.personEmail == 'Kathleen.Baert@kohls.com') {
	    client.createMessage(command.message.roomId, "no u", { "markdown":"true" }, function(err, response) {
            if (err) {
    ...
    var options = {
        ...
        'path': '/v1/gifs/search?api_key=fwAMRmabyfhzMFQXmOmaQf6KIAIt3zrR&q=' + command.keyword + '&limit=10&offset=0&rating=PG-13&lang=en'
    };
    ...
    var req = https.request(options, function (response) {
	    ...
	    {	
                client.createMessage(command.message.roomId, "Stay thirsty, my friend.");
     	        // client.createMessage(command.message.roomId, "", { "file": data[Math.floor(Math.random()*data.length)].images.downsized.url }, function(err, response) {
       	        //     if (err) {
    ...
    req.on('error', function(err) {
        console.log("WARNING: could not complete api call." + err);
	client.createMessage(command.message.roomId, "Sorry, I couldn't find any gifs... try searching with a new keyword or phrase.",
		{ "markdown":"true" }, function(err, response) {
	    if (err) {
	        console.log("WARNING: Could not post fallback message after finding no gifs." + command.message.roomId);
